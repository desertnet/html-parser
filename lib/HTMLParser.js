...
import {NodeType} from './HTMLNode'
import RootNode from './HTMLNode/RootNode'
 ...
 * string of HTML, and it will return a Foundaiton.HTML.Parser.HTMLNode
 ...
 * Pushing a node adds a new Foundation.HTML.Parser.HTMLNode to the
...
export default class HTMLParser {
  ...
  /**
   * @deprecated Extra validations are now standard.
   * @public
  ...
  finalize () {
    ...
    while (this._nodes.length) {
      ...
      if (!addedErrorToTopmostNode) {
        ...
        error.message = "Unexpected end of HTML.";
  ...
  popNode () {
    if (this._nodes.length === 0) {
      throw new Error("HTMLNode stack is unexpectedly empty.");
    }
  ...
  addClosedElementToParent (element) {
    ...
    closedElements.forEach(element => {
      if (!element.closingTag()) {
        var error = new HTMLParseError();
        error.message = `Could not find closing tag for "<${element.tagName}>".`;
  ...
  /**
   * @private
   * @param {Foundation.HTML.Parser.HTMLNode} elementNode
   */
